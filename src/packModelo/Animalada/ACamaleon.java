	...
	public void hacerAnimalada(int pFuerza, String pColor) {
		...
		if (BarBestial.getBarBestial().esTurnoJugador()) {
			if (!ColaEntrada.getColaEntrada().colaVacia()) {
				while (!correcto) {
					try {
						pPos = Integer.parseInt(JOptionPane.showInputDialog(frame, "Introduzca posicion en la cola",
								"Camaleon", JOptionPane.INFORMATION_MESSAGE));
					} catch (Exception e) {

						JOptionPane.showMessageDialog(frame, "Introduzca un numero valido, por favor", "Camaleon",
				...
				JOptionPane.showMessageDialog(frame, "No hay elementos en la cola para elegir", "Camaleon",
						JOptionPane.INFORMATION_MESSAGE);
			}
		} else {
			if (!ColaEntrada.getColaEntrada().colaVacia()) {
